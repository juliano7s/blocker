...
public partial class GridRenderer : Node2D
{
    ...
    private void DrawOuterCornerTicks(Rect2 rect, float len, Color color, float width)
    {
        ...
        // Top-left corner — extend each line past the junction to close the gap
        DrawLine(tl + new Vector2(-diag - hw, -diag), tl + new Vector2(len, -diag), color, width);
        DrawLine(tl + new Vector2(-diag, -diag - hw), tl + new Vector2(-diag, len), color, width);
        // Top-right corner
        DrawLine(tr + new Vector2(diag + hw, -diag), tr + new Vector2(-len, -diag), color, width);
        DrawLine(tr + new Vector2(diag, -diag - hw), tr + new Vector2(diag, len), color, width);
        // Bottom-left corner
        DrawLine(bl + new Vector2(-diag - hw, diag), bl + new Vector2(len, diag), color, width);
        DrawLine(bl + new Vector2(-diag, diag + hw), bl + new Vector2(-diag, -len), color, width);
        // Bottom-right corner
        DrawLine(br + new Vector2(diag + hw, diag), br + new Vector2(-len, diag), color, width);
        DrawLine(br + new Vector2(diag, diag + hw), br + new Vector2(diag, -len), color, width);
    }
    ...
    private void DrawSoldierAnimated(Block block, Rect2 rect, Vector2 center, PlayerPalette palette, float idleAngle)
    {
        ...
        if (armTL) QueueGlowLine(tl, center, glowColor, 1.5f, false);
        if (armBR) QueueGlowLine(center, br, glowColor, 1.5f, false);
        if (armTR) QueueGlowLine(tr, center, glowColor, 1.5f, false);
        if (armBL) QueueGlowLine(center, bl, glowColor, 1.5f, false);

        // Core swords (thin, solid)
        if (armTL) DrawLine(tl, center, gold, 1.4f);
        if (armBR) DrawLine(center, br, gold, 1.4f);
        if (armTR) DrawLine(tr, center, gold, 1.4f);
        if (armBL) DrawLine(center, bl, gold, 1.4f);
